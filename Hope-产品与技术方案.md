...
- [x] `expiredBehaviors` 设置项（**三个独立勾选框**：闪烁提醒 / 全屏庆祝 / 系统通知，可叠加；默认全空＝到期自动显示）——此为全局默认值
- [x] 任务编辑区「到期提醒」：**「使用全局默认」** 勾选框 + 三项叠加勾选框。勾「使用全局默认」时 `expiredBehaviors=null`（沿用全局）并禁用三项；取消后可独立叠加。**新建任务默认「使用全局默认」**
...
- [x] 过期 `behaviors` 含 `blink`（或 `celebrate`）：该到期保留段做**柔和 alpha 渐变脉冲**（`DoubleAnimation`，正弦缓动淡出/淡入），**持续到任务不再到期**（重新设定时间 / 删除 / 完成）；打开/激活设置窗口**不**停止呼吸
...
- **闪烁矩形持久化（避免被无关刷新打断）**：
  - 问题：旧实现 `Render()` 在签名变化时整体 `BarCanvas.Children.Clear()` 重建所有矩形。由于签名涵盖**所有段**的 `FillEnd`，未完成任务每秒推进进度即触发整条重建，已完成段的闪烁矩形被销毁重建、`StartBlink` 从相位 0 重启，导致闪烁被打断、重置。
  - 方案：将「正在闪烁的到期段」矩形按 `taskId`（每个 `OverlayWindow` 对应一条边，故等价于 `taskId`）缓存复用（参照 `ImageSprite` 的 `_sprites` 做法）。`Render()` 不再无脑全清：
    - 对**仍在闪烁且几何（`BarStart/BarEnd/FillEnd`）、颜色、到期态未变**的段——复用既有矩形与**正在运行的渐变动画**，不重建、不重启；仅按需更新位置/尺寸。
    - 对**新进入闪烁**或几何/颜色变化的闪烁段——才创建矩形并 `StartBlink`（多任务时仍按 `taskId` 排序错峰、加长渐变）。
    - 对**非闪烁段**（含逐秒推进的未完成段）——照常重建，不影响上面的持久化闪烁矩形。
    - 任务**不再到期 / 段消失**时，移除其缓存矩形并停止动画。
  - 效果：未完成任务进度刷新与已完成任务闪烁彼此**完全隔离**，闪烁相位连续不重置。
  - **几何与动画解耦**：多任务在主条上按填充量紧挨拼接，进行中任务推进会令已完成段像素平移；复用判定区分「几何签名」（变化只搬动矩形）与「动画签名 `half:delay`」（仅此变化才重启动画），避免几何平移导致呼吸被每秒重置。
- [x] **跨窗口相位同步（非四边环绕 + 全屏庆祝）**：主条与三个氛围条是 4 个独立 `OverlayWindow`，各自独立动画时钟。`StartBlink` 以 App 启动的全局锚点 `BlinkAnchorUtc` 按绝对墙钟计算当前相位、用**负 `BeginTime`** 对齐——各窗口无论何时启动动画都落在同一相位，四条边同步呼吸。多任务错峰延时在整个呼吸周期 `2×half` 内按任务数均匀铺开（2 个任务即反相、明暗交替明显）。
- [x] **打开设置不停止呼吸**：打开/激活设置窗口不再触发 `AcknowledgeBlink`，主条与三个氛围条持续同步呼吸，直到任务本身不再到期。（`AcknowledgeBlink` 仍保留为可选 API，当前无调用点。）
...
- **UI 形态**：全局设置与任务编辑均用 **三个独立勾选框**（闪烁提醒 / 全屏庆祝 / 系统通知），可任意叠加；空集合 = 仅自动显示。
- **全局默认 + 任务级**：
  - 全局设置中的 `expiredBehaviors` 为 **默认值**（默认空集合）。
  - 任务编辑区「到期提醒」含 **「使用全局默认」** 勾选框：勾选时 `task.expiredBehaviors = null`，运行期沿用全局，且本任务三项勾选框 **禁用**；取消勾选后可独立叠加三项（各存其值）。
  - **新建任务默认勾「使用全局默认」**（动态跟随全局）。
