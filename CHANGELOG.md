...
- **高级设置**：新增进度条方向选择（forward/reverse），默认智能设定
- **四边模式**：高级设置中新增"我全都要"选项，启用后进度条同时显示在屏幕四边
...
- `Views.ConfigWindow` 新增 UI：
  - 全局设置：进度条位置下拉
  - 高级设置：进度条方向下拉、四边模式复选框、允许任务级位置覆盖复选框
...
- `src/headless/`：engine、task、config、ipc 包
